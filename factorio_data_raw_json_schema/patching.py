...
# I believe 'DamageEntityTriggerEffectItem' is a typo in https://lua-api.factorio.com/2.0.32/types/TriggerEffect.html
...
# For example:
#   cat script-output/data-raw-dump.json | jq '."assembling-machine"."captive-biter-spawner".allowed_effects'
...
def patch_doc(doc: documentation.Doc) -> None:
    ...
    #   cat script-output/data-raw-dump.json | jq '."mining-drill"."electric-mining-drill".graphics_set.shift_animation_waypoint_stop_duration'
    ...
    # but is some kind of floating point number in, for example:
    #   cat script-output/data-raw-dump.json | jq '.recipe."accumulator-recycling".results[0].amount'
    ...
    #   cat script-output/data-raw-dump.json | jq '."gui-style".default.technology_slot.level_offset_y'
    ...
    # but is some kind of floating point number in, for example:
    #   cat script-output/data-raw-dump.json | jq '."gui-style".default.technology_slot.level_range_offset_y'
    doc.get_type_def("TechnologySlotStyleSpecification", documentation.StructTypeExpression).get_property_type(
    ...
    # but is some kind of floating point number in, for example:
    #   cat script-output/data-raw-dump.json | jq '."segmented-unit"."big-demolisher".revenge_attack_parameters.lead_target_for_projectile_delay'
    ...
    #   cat script-output/data-raw-dump.json | jq '."mining-drill"."big-mining-drill".graphics_set.working_visualisations'
    # Note that is is an array as expected in, for example:
    #   cat script-output/data-raw-dump.json | jq '."mining-drill"."electric-mining-drill".graphics_set.working_visualisations'
    ...
    #   cat script-output/data-raw-dump.json | jq '."agricultural-tower"."agricultural-tower".crane.parts[0].dying_effect.particle_effects'
    ...
    #   cat script-output/data-raw-dump.json | jq '.shortcut.redo.action'
    ...
    # but that value is used in, for example:
    #   cat script-output/data-raw-dump.json | jq '."dont-build-entity-achievement"."logistic-network-embargo".objective_condition'
    ...
    #   cat script-output/data-raw-dump.json | jq '."utility-constants".default'
    ...
    # but is absent from, for example:
    #   cat script-output/data-raw-dump.json | jq '."space-location"."space-location-unknown"'
    ...
    # but is absent from, for example:
    #   cat script-output/data-raw-dump.json | jq '."editor-controller".default'
    ...
    # but is absent from, for example:
    #   cat script-output/data-raw-dump.json | jq '."dont-kill-manually-achievement"."keeping-your-hands-clean"'
    doc.get_prototype("AchievementPrototypeWithCondition").get_property("objective_condition").required = False

    # https://lua-api.factorio.com/2.0.32/prototypes/UtilitySprites.html#cursor_box documents attribute 'rts_selected' as required
    # but it's absent from, for example:
    #   cat script-output/data-raw-dump.json | jq '."utility-sprites".default.cursor_box'
    ...
    # https://lua-api.factorio.com/2.0.32/prototypes/UtilitySprites.html#cursor_box documents attribute 'rts_to_be_selected' as required
    # but it's absent from, for example:
    #   cat script-output/data-raw-dump.json | jq '."utility-sprites".default.cursor_box'
    doc.get_prototype("UtilitySprites").get_property_type(
        "cursor_box", documentation.StructTypeExpression
    ...
    # https://lua-api.factorio.com/2.0.32/types/SingleGraphicProcessionLayer.html#frames documents attribute 'frame' as required
    # but it's absent from, for example:
    #   cat script-output/data-raw-dump.json | jq '.procession."platform-to-planet-a".timeline.layers[5].frames[0]'
    typing.cast(
        documentation.StructTypeExpression,
    ...
    # but is absent from, for example:
    #   cat script-output/data-raw-dump.json | jq '.procession."default-intermezzo".timeline'
    ...
    # but is absent from, for example:
    #   cat script-output/data-raw-dump.json | jq '."rail-ramp"."dummy-rail-ramp".pictures'
    ...
    # but is absent from, for example:
    #   cat script-output/data-raw-dump.json | jq '.wall."stone-wall".pictures.ending_left'
    doc.get_type_def("SpriteSource", documentation.StructTypeExpression).get_property("filename").required = False

    # https://lua-api.factorio.com/2.0.32/types/NeighbourConnectableConnectionDefinition.html#location documents attributes 'direction' as a 'MapPosition'
    # but it's an integer in, for example:
    #   cat script-output/data-raw-dump.json | jq '."fusion-reactor"."fusion-reactor".neighbour_connectable.connections[0].location.direction'
    ...
    # https://lua-api.factorio.com/2.0.32/types/SingleGraphicProcessionLayer.html#frames documents attribute 'timestamp' as 'MapTick', which is an alias for 'uint64'
    # but it's some kind of floating point number in, for example:
    #   cat script-output/data-raw-dump.json | jq '.procession."planet-to-platform-a".timeline.layers[33].frames[0].timestamp'
    typing.cast(
        documentation.StructTypeExpression,
        doc.get_type_def("SingleGraphicProcessionLayer", documentation.StructTypeExpression)
        .get_property_type("frames", documentation.ArrayTypeExpression)
    ...
    #   cat script-output/data-raw-dump.json | jq '."rocket-silo"."rocket-silo".alarm_sound'
    ...
    # but is absent from, for example:
    #   cat script-output/data-raw-dump.json | jq '.turret."medium-worm-turret".spawn_decoration[0]'
    # (and many others)
    ...
    # but is absent from, for example:
    #   cat script-output/data-raw-dump.json | jq '.car.tank.track_particle_triggers[2]'
    ...
    # but is some kind of floating point number in, for example:
    #   cat script-output/data-raw-dump.json | jq '.segment."medium-demolisher-segment-x0_6525".update_effects[1].effect[0].repeat_count'
    ...
    # but is some kind of floating point number in, for example:
    #   cat script-output/data-raw-dump.json | jq '."optimized-particle"."vulcanus-stone-particle-smoke-small".ended_in_water_trigger_effect[1].tail_length_deviation'
    ...
    # but is some kind of floating point number in, for example:
    #   cat script-output/data-raw-dump.json | jq '."chain-active-trigger"."chain-tesla-turret-chain".action.action_delivery.max_length'
